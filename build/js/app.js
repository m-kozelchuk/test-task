...
function _toConsumableArray(arr) { if (Array.isArray(arr)) { for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) { arr2[i] = arr[i]; } return arr2; } else { return Array.from(arr); } } // some code
...
document.addEventListener("DOMContentLoaded", function () {
  new _swiper2.default('.swiper-banner', {
    loop: true,
    speed: 800,
    effect: 'fade',
    fadeEffect: {
  ...
  new _swiper2.default('.swiper-portfolio', {
    loop: true,
    speed: 800,
    spaceBetween: 100,
    pagination: {
  ...
  var target = document.querySelectorAll('.js-block-animate');
  ...
  [].concat(_toConsumableArray(target)).forEach(function (el) {
    observer.observe(el);
  });
});
...
function select() {
  ...
  Array.prototype.forEach.call(select, function (el) {
    ...
    var template = '<span class="select-trigger">' + placeholder + '</span>';
    ...
    var trigger = el.closest('.select-wrap').querySelector('.select-trigger');
    ...
    [].concat(_toConsumableArray(options)).forEach(function (opt) {
      opt.addEventListener('click', function () {
        var select = this.closest('.select-wrap'),
            val = this.getAttribute('data-value');

        var selects = select.querySelector('select');
        [].concat(_toConsumableArray(selects)).forEach(function (sl) {
          sl;
        });

        var opts = select.querySelectorAll('option');
        [].concat(_toConsumableArray(opts)).forEach(function (o) {
  ...
  document.documentElement.addEventListener('click', function () {
    ...
    [].concat(_toConsumableArray(selectNodes)).forEach(function (node) {
...
var next = document.querySelector('.link_next');
scroll(main, next);
...
function bgTextParallax() {
  ...
  [].concat(_toConsumableArray(bgText)).forEach(function (el) {
    var yPos = window.pageYOffset / el.dataset.speed;
    var coords = yPos + '%';
    el.style.top = coords;
  });
}
